...
final class ParametersManager {
    ...
    static let shared: ParametersManager = ParametersManager()
    
    var minHourContactNotif: Int? {
        guard let hour = valueFor(name: "app.minHourContactNotif") as? Double else { return nil }
        return Int(hour)
    }
    var maxHourContactNotif: Int? {
    ...
    var bleServiceUuid: String? { valueFor(name: "ble.serviceUUID") as? String }
    var bleCharacteristicUuid: String? { valueFor(name: "ble.characteristicUUID") as? String }
    
    private let url: URL = URL(string: "https://app.stopcovid.gouv.fr/json/version-22/config.json")!
    ...
    func getDeviceParametersFor(model: String) -> DeviceParameters? {
        ...
        if let parameters = devicesParameters.filter({ $0.model == model }).first {
            ...
            let txAverage: Double = -6.52
            let rxAverage: Double = -19.71
    ...
    private func createWorkingDirectoryIfNeeded() -> URL {
        let directoryUrl: URL = FileManager.libraryDirectory().appendingPathComponent("config")
